...
!(function(t, e) {
  ...
  function t(t, n, r) {
    var i = e.uniformDiscrete(89, 91) / 100,
      ...
      d =
        (n.width - Math.sqrt(Math.pow(t.width, 2) + Math.pow(t.height, 2))) /
          2 -
        h,
...
Promise.all(assets).then(function(images) {
  ...
  canvas.style['top'] = '0';
  canvas.style['right'] = '0rem';
  canvas.style['zIndex'] = '998';
  canvas.style['width'] = '10rem';
  canvas.style['height'] = '33vh';
  document.body.appendChild(canvas);
  //journal-reward 为点击的按钮
  document.getElementsByClassName('heart')[0].addEventListener(
    'click',
    function() {
      //随机飘动
